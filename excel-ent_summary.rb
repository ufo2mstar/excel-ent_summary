...
output_file  ="scen_UAT-reg_#{Time.now.strftime '%F_%H-%M-%S'}.xls"

# What to look for <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
#1.0=["TX", "TN", "PA", "NY", "VA"]
#3.0=["AL", "CT", "GA", "IL", "MD", "OH"]
#6.0=["KY", "MS", "NC", "SC", "WV", "DE"]
#9.0=["AR", "DC", "ME", "NH", "RI", "VT"]
#  TX,TN,PA,NY,VA,AL,CT,GA,IL,MD,OH,KY,MS,NC,SC,WV,DE,AR,DC,ME,NH,RI,VT # all 23 states PQQ
#  AZ,CA,CO,ID,IA,KS,MN,MO,NE,NV,OR,UT,WA,WI # all 14 states HO_WEST
#  CA,AZ,CO,MN,MO,WA,OR,KS,UT,WI,NE,NV,IA,ID # all 14 states RQB_WEST
#  AR,GA,MS # all 3 states RQB_EAST

states       = %w[AR GA MS IN CA] # Renters
...
globbed_res  = Dir.glob(dir_loc).reject { |x| x =~ /old/ }
